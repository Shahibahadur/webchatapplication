...
public class SearchRequestHandler extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//printing session
		System.out.println("Session from searchrequest handler "+session.getAttribute("unique_id"));
		...
		try {
			...
			while(set.next()) {
				...
				//testing 
				System.out.println("sender_id must be similar to Session from searchrequest handler "+sender_id);
				...
				if(set.getString("status").equalsIgnoreCase("Offline now")) {
					...
					offline = "";
					   ...
					   System.out.println("getting unique id from other user from ssearchrequest handler "+set.getString("unique_id"));
					   output += "<a href=\"http://localhost:8080/ChatAPP/chats?user_id="+ set.getString("unique_id") +"\">\n"
